...
const testimonials = [
  "This is what I wish I knew before applying.",
  "Finally, training that explains the mine before the machine.",
  "It made the whole operator pathway feel real and achievable.",
];
...
export default function MiningAcademyLandingPage() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white">
      <style>{`section[id] { scroll-margin-top: 110px; }`}</style>
      ...
      <nav className="fixed left-0 right-0 top-0 z-50 px-4 pt-4">
        <div className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-white/10 bg-black/65 px-4 py-3 shadow-[0_20px_80px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          ...
          <Button href="#offer" className="px-5 py-3">Join Now</Button>
        </div>
      </nav>

      <section id="top" className="relative min-h-screen border-b border-white/10 pt-24">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_20%,rgba(217,181,109,0.2),transparent_38%),linear-gradient(180deg,rgba(0,0,0,0.2),#050505_88%)]" />
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl items-start gap-14 px-6 pb-20 pt-20 lg:grid-cols-2 lg:pt-28">
          <div>
            ...
            <h1 className="text-5xl font-black leading-[0.9] tracking-[-0.06em] md:text-7xl lg:text-8xl">
              Break Into Mining With Operator-Level Confidence.
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-relaxed text-white/68 md:text-xl">
              A premium online training system built to help beginners understand mine sites, haul truck operation, safety expectations, and the real language of the industry before they ever step on site.
            </p>
            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              <Button href="#offer" className="h-14 px-8 text-base">Start Training <Icon name="arrow" className="ml-2 h-5 w-5" /></Button>
              <Button href="#training" variant="outline" className="h-14 px-8 text-base"><Icon name="play" className="mr-2 h-5 w-5" /> View Program</Button>
            </div>
            <div className="mt-10 flex flex-wrap gap-3 text-sm text-white/60">
              <span className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-2">Zero experience friendly</span>
              <span className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-2">Safety-first training</span>
              <span className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-2">Community included</span>
            </div>
          </div>
      ...
      <section className="border-y border-white/10 bg-[#0a0a0a] py-28">
        <div className="mx-auto grid max-w-7xl items-center gap-16 px-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-6 text-lg leading-relaxed text-white/62">The goal is to help you sound, think, and prepare like someone who understands the job before the first day.</p>
          </div>
      ...
      <section id="community" className="border-y border-white/10 bg-[#090909] py-28">
        <div className="mx-auto grid max-w-7xl items-center gap-16 px-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div>
            ...
            <p className="mt-6 text-lg leading-relaxed text-white/62">The community gives students a focused space to ask questions, stay accountable, understand the industry, and build confidence around others serious about the same goal.</p>
          </div>
        </div>
      </section>
      ...
      <section id="results" className="py-28">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mx-auto mb-14 max-w-3xl text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#d9b56d]">Results</p>
            <h2 className="mt-5 text-4xl font-black tracking-[-0.05em] md:text-6xl">Built for confidence before the first shift.</h2>
          </div>
            ...
            {testimonials.map((quote) => (
              <Card key={quote} className="text-white">
                <div className="p-8">
                  ...
                  <p className="mt-8 text-sm text-white/40">Early student feedback</p>
                </div>
              </Card>
            ))}
          </div>
      ...
      <section id="offer" className="border-t border-white/10 bg-[#0a0a0a] py-28">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#d9b56d]">Enrollment</p>
          <h2 className="mt-5 text-4xl font-black leading-[0.92] tracking-[-0.06em] md:text-7xl">Start building your mining career before your first shift.</h2>
          <div className="mt-12 rounded-[2.25rem] border border-[#d9b56d]/25 bg-[radial-gradient(circle_at_50%_0%,rgba(217,181,109,0.17),transparent_35%),#080808] p-6 text-left shadow-2xl md:p-10">
            <div className="flex flex-col gap-6 border-b border-white/10 pb-8 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-3 text-white/55">Training, community, updates, and operator pathway support.</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-white/40">Founding Member</p>
                <p className="text-5xl font-black tracking-[-0.06em]">$---</p>
              </div>
            </div>
            ...
            <Button href="#offer" className="mt-10 h-16 w-full text-lg">Join Mining Academy <Icon name="arrow" className="ml-2 h-5 w-5" /></Button>
            <p className="mt-5 text-center text-xs text-white/35">Built for serious beginners who want to enter mining with confidence.</p>
          </div>
        </div>
      </section>
